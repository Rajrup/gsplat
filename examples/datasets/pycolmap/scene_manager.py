            ...
            for line in iter(lambda: f.readline().strip(), ''):
                ...
                if is_camera_description_line:
                    ...
                    image = Image(
                        ...
                        int(data[-2]),
                        Quaternion(
                            np.fromiter(map(float, data[1:5]), dtype=np.float64)
                        ),
                        np.fromiter(map(float, data[5:8]), dtype=np.float64),
                    )
                else:
                    image.points2D = np.array(
                        [
                            np.fromiter(map(float, data[::3]), dtype=np.float64),
                            np.fromiter(map(float, data[1::3]), dtype=np.float64),
                        ]
                    ).T
                    image.point3D_ids = np.fromiter(
                        (
                            SceneManager.INVALID_POINT3D if v == -1 else v
                            for v in map(int, data[2::3])
                        ),
                        dtype=np.uint64,
                    )
        ...
        with open(input_file, 'r') as f:
            for line in iter(lambda: f.readline().strip(), ''):
                ...
                self.points3D.append(
                    np.fromiter(map(np.float64, data[1:4]), dtype=np.float64)
                )
                self.point3D_colors.append(
                    np.fromiter(map(np.uint8, data[4:7]), dtype=np.uint8)
                )
                self.point3D_errors.append(np.float64(data[7]))

                # load (image id, point2D idx) pairs
                self.point3D_id_to_images[point3D_id] = \
                    np.fromiter(map(np.uint32, data[8:]), dtype=np.uint32).reshape(-1, 2)
