...
class Runner:
    ...
    def render_traj(self, step: int):
        ...
        writer = imageio.get_writer(f"{video_dir}/traj_{step}.mp4", fps=cfg.render_traj_fps)
...
def main2(local_rank: int, world_rank, world_size: int, cfg: Config):
    ...
    if cfg.run_mode == "train":
        ...
        elif cfg.run_mode == "resume":   # resume training
            for k in runner.splats.keys():
                runner.splats[k].data = torch.cat([ckpt["splats"][k]])
            if cfg.pose_opt and "pose_adjust" in ckpt:
                if world_size > 1:
                    runner.pose_adjust.module.load_state_dict(ckpt["pose_adjust"])
                else:
                    runner.pose_adjust.load_state_dict(ckpt["pose_adjust"])
            if cfg.app_opt and "app_module" in ckpt:
                if world_size > 1:
                    runner.app_module.module.load_state_dict(ckpt["app_module"])
                else:
                    runner.app_module.load_state_dict(ckpt["app_module"])
            runner.train(step=ckpt["step"]+1)
        elif cfg.run_mode == "ft":  # fine-tuning
            if cfg.flag_concat_splats: # Concatenate the checkpoint Gaussians with the new Gaussians
                for k in runner.splats.keys():
                    if k in ckpt["splats"]:
                        runner.splats[k].data = torch.cat([
                            ckpt["splats"][k],
                            runner.splats[k].data
                        ], dim=0)
            else:
                for k in runner.splats.keys():
                    runner.splats[k].data = torch.cat([ckpt["splats"][k]])
