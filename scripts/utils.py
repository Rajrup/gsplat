...
def extract_path_components(data_dir_path):
    """
    Extract Actor01, Sequence1, and 4x from path like:
    /synology/actorshq/colmap/Actor01/Sequence1/4x/frames/frame0
    """
    parts = data_dir_path.strip('/').split('/')
    
    # Prefer ActorsHQ layout rooted at 'colmap'
    try:
        colmap_index = parts.index('colmap')
        if colmap_index + 3 < len(parts):
            actor_name = parts[colmap_index + 1]  # Actor01
            sequence_name = parts[colmap_index + 2]  # Sequence1
            resolution_name = parts[colmap_index + 3]  # 4x
            return actor_name, sequence_name, resolution_name
    except (ValueError, IndexError):
        pass

    # Fallback to older layout rooted at 'data'
    try:
        data_index = parts.index('data')
        if data_index + 4 < len(parts):
